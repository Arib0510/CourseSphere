...
import { generateExamFormPDF } from '../../utils/generateExamFormPDF'
...
export default function RegistrationsPage() {
  ...
  const [generatingExamPDF, setGeneratingExamPDF] = useState(false)
  const [examFormModal, setExamFormModal] = useState(false)
  ...
  const deptToBengali = (dept) => {
    const map = {
      CSE: 'সিএসই', EEE: 'ইইই', CE: 'পুর', ME: 'যন্ত্র',
      IPE: 'আইপিই', GCE: 'জিসিই', MSE: 'এমএসই', ECE: 'ইসিই',
      URP: 'ইউআরপি', Arch: 'স্থা', CHE: 'রসায়ন', MTE: 'এমটিই',
    }
    return map[dept] || dept || ''
  }

  const handleExamFormButtonClick = () => {
    if (registrations.length === 0) {
      toast.info('No registered courses to include in the exam form.')
      return
    }
    setExamOptions(prev => ({ ...prev, koushal: deptToBengali(user?.department) }))
    setExamFormModal(true)
  }

  const handleGenerateExamPDF = async () => {
    setExamFormModal(false)
    setGeneratingExamPDF(true)
    try {
      await generateExamFormPDF(user, registrations, examOptions)
      toast.success('Exam form downloaded!')
    } catch {
      toast.error('Failed to generate exam form. Please try again.')
    } finally {
      setGeneratingExamPDF(false)
    }
  }
    ...
    <div className="pt-4 space-y-8">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {/* PDF buttons — only show when there are registrations */}
          {registrations.length > 0 && (
            <>
              ...
              >
                ...
                {generatingPDF ? 'Generating…' : 'Registration Form'}
              </button>

              <button
                onClick={handleExamFormButtonClick}
                disabled={generatingExamPDF}
                className="inline-flex items-center gap-2 px-4 py-2 bg-secondary text-on-secondary rounded-full font-body-md-bold text-sm shadow-sm hover:bg-secondary/90 active:scale-95 transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed"
              >
                <span className="material-symbols-outlined text-[18px]" style={{ fontVariationSettings: "'FILL' 1" }}>
                  {generatingExamPDF ? 'progress_activity' : 'assignment'}
                </span>
                {generatingExamPDF ? 'Generating…' : 'Exam Form'}
              </button>
            </>
          )}
        </div>
      ...
      {/* Exam Form Options Modal */}
      <Modal
        isOpen={examFormModal}
        onClose={() => setExamFormModal(false)}
        title="পরীক্ষার ফর্মের তথ্য পূরণ করুন"
        size="md"
      >
        <div className="space-y-4">
          {/* Exam year */}
          <div>
            <label className="block text-sm font-body-md text-on-surface-variant mb-1">
              পরীক্ষার সাল <span className="text-xs text-on-surface-variant">(২০-এর পরের অংশ, যেমন: ২৬)</span>
            </label>
            <input
              type="number"
              min="0" max="99"
              value={examOptions.examYear}
              onChange={e => setExamOptions(prev => ({ ...prev, examYear: e.target.value.slice(0, 2) }))}
          ...
          {/* Study year */}
          <div>
            <label className="block text-sm font-body-md text-on-surface-variant mb-1">বর্ষ</label>
            <select
              value={examOptions.studyYear}
              onChange={e => setExamOptions(prev => ({ ...prev, studyYear: e.target.value }))}
              className="w-full border border-outline-variant rounded-lg px-3 py-2 text-on-background font-body-md focus:outline-none focus:ring-2 focus:ring-primary"
            >
              <option value="১ম">১ম বর্ষ</option>
              <option value="২য়">২য় বর্ষ</option>
              <option value="৩য়">৩য় বর্ষ</option>
              <option value="৪র্থ">৪র্থ বর্ষ</option>
            </select>
          </div>

          {/* Koushal */}
          <div>
            <label className="block text-sm font-body-md text-on-surface-variant mb-1">কৌশল (বিভাগ)</label>
            <input
              type="text"
              value={examOptions.koushal}
              onChange={e => setExamOptions(prev => ({ ...prev, koushal: e.target.value }))}
              className="w-full border border-outline-variant rounded-lg px-3 py-2 text-on-background font-body-md focus:outline-none focus:ring-2 focus:ring-primary"
              placeholder="সিএসই"
            />
          </div>
          ...
          {/* Semester no */}
          <div>
            <label className="block text-sm font-body-md text-on-surface-variant mb-1">সেমিস্টার নং</label>
            <select
              value={examOptions.semesterNo}
          ...
          {/* Exam type */}
          <div>
            <label className="block text-sm font-body-md text-on-surface-variant mb-2">পরীক্ষার ধরন</label>
            <div className="flex flex-wrap gap-4">
              {[
                { value: 'semester', label: 'সেমিস্টার' },
                { value: 'backlog',  label: 'ব্যাকলগ' },
                { value: 'short',    label: 'শর্ট সেমিস্টার' },
              ].map(opt => (
                <label key={opt.value} className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    name="examType"
                    value={opt.value}
                    checked={examOptions.examType === opt.value}
                    onChange={() => setExamOptions(prev => ({ ...prev, examType: opt.value }))}
          ...
          <div className="flex gap-3 justify-end pt-2 border-t border-outline-variant">
            <Button variant="ghost" onClick={() => setExamFormModal(false)}>বাতিল</Button>
            <Button onClick={handleGenerateExamPDF}>
              <span className="material-symbols-outlined text-[18px]" style={{ fontVariationSettings: "'FILL' 1" }}>assignment</span>
              ফর্ম তৈরি করুন
            </Button>
          </div>
        </div>
